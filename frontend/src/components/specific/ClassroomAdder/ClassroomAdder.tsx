...
function ClassroomAdder() {
  ...
  async function addClassroom() {
    ...
    const files = materials.map(async (file) => {
      const base64 = await convertToBase64(file);

      // Remove the "data:application/pdf;base64," part
      const base64Data = base64.split(",")[1];

      return { filename: file.name, fileContent: base64Data };
    });

    const data = await post("http://localhost:8060/classroom", {
      title: "title",
      description: "description",
      isPrivate: true,
      password: "dfsfsdfsdfs",
      files: files,
      date: "2024-01-01T14:30:30",
    });
    ...
    //       title: title,
    //       description: description,
    //       startTime: newDate.toDateString(),
    //       private: !isPublic,
