...
function handleLastName(e:any){
    ...
    useEffect(()=>{
        if(!confirmPassword.length){
         ...
         if(deadItem3.current){
             deadItem3.current.style.setProperty('transform', 'translate(-40%, -379%)', 'important');
    ...
    useEffect(()=>{
        if(!firstName.length){
         ...
         if(deadItem4.current){
             deadItem4.current.style.setProperty('transform', 'translate(-0%, -379%)', 'important');
    ...
    useEffect(()=>{
        if(!lastName.length){
         ...
         if(deadItem5.current){
             deadItem5.current.style.setProperty('transform', 'translate(-0%, -379%)', 'important');
    ...
    async function register(){
        ...
        let res=await post('http://localhost:8060/auth/register',{email:"anes@gmail.com",password:"12345678",firstName:"azd",lastName:"azdad"})
        if(res.status==200){
            ...
            <main id={styles.loginMain}>
                <section id={styles.loginSection}>
                    ...
                    <form id={styles.loginForm}>
                        ...
                        <input  onChange={handleFirstName} value={firstName} id={styles.namesInput} type="text" />
                        <p  ref={deadItem4} id={styles.deadItem4}>Your Firstname</p>
                        </div>
                        <div className={styles.inputSpan}>
                        <input  onChange={handleLastName} value={lastName} id={styles.namesInput} type="text" />
                        <p  ref={deadItem5} id={styles.deadItem5}>Your Lastname</p>
                        </div>
                        ...
                        <div id={styles.loginControls}>
                            ...
                            <button type="button" onClick={register} className={styles.authBtn} id={styles.loginBtn}>Register</button>
                        </div>
